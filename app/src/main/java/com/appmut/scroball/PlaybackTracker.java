...
public class PlaybackTracker {
    ...
    public void handlePlaybackStateChange(String player, PlaybackState playbackState) {
        Log.v("WICHTIG!", "Playback change: " + player);
    ...
    if(playerState.isPaused){
      ...
      if(PlaybackTracker.scheduledFuture != null){
        ...
        if (metadata.getString(MediaMetadata.METADATA_KEY_TITLE).equals("WDR - 1Live") && !playerState.isPaused) {
            return;
            ...
            if (PlaybackTracker.pollingTaskRunning && scheduledFuture != null) {     //Kai
                PlaybackTracker.scheduledFuture.cancel(false);
                PlaybackTracker.pollingTaskRunning = false;
                LastfmClient.loglog.add("polling Task stopped: new metadata: " + metadata.getString(MediaMetadata.METADATA_KEY_TITLE));
                if (wakeLock.isHeld()) {
                    wakeLock.release();
                }
            }
...
/*
    ...
    private PlayerState getOrCreatePlayerState(String player) {
        ...
        if (!playerStates.containsKey(player)) {
            playerState = new PlayerState(player, scrobbler, scrobbleNotificationManager, wakeLock, this);
    ...
    public void pollingTask(String player) {
